use {
    crate::{
        ...
        AsContextMut, CallHook, Engine, StoreContextMut, ValRaw,
    },
    ...
    match resume_fiber(&mut fiber, Some(store), Ok(()))? {
        Ok((mut store, result)) => {
            ...
            if async_ {
                ...
                if !(matches!(&task.caller, Caller::Guest {..} if task.result.is_some())
...
fn resume_stackless<'a, T>(
    ...
    let event = if task.lift_result.is_some() {
        ...
        Event::Returned
    } else if matches!(&task.caller, Caller::Guest {..} if task.result.is_some())
        || matches!(&task.caller, Caller::Host(tx) if tx.is_none())
    {
        Event::Done
    } else {
...
unsafe fn call_host_and_handle_result<T, R: crate::runtime::vm::HostResultHasUnwindSentinel>(
    cx: *mut VMOpaqueContext,
    func: impl FnOnce() -> Result<R>,
) -> R::Abi {
    let cx = VMComponentContext::from_opaque(cx);
    let instance = (*cx).instance();
    let raw_store = (*instance).store();
    let store = StoreContextMut::<T>(&mut *raw_store.cast());
...
pub(crate) extern "C" fn task_backpressure<T>(
    ...
    unsafe {
        call_host_and_handle_result::<T, _>(cx, || {
            let cx = VMComponentContext::from_opaque(cx);
            let instance = (*cx).instance();
            let mut cx = StoreContextMut::<T>(&mut *(*instance).store().cast());
            let entry = cx
...
pub(crate) extern "C" fn task_yield<T>(cx: *mut VMOpaqueContext, async_: bool) -> bool {
    unsafe {
        call_host_and_handle_result::<T, _>(cx, || {
            task_check::<T>(cx, async_, TaskCheck::Yield)?;
            Ok(())
        })
    }
}
                ...
                if let Some(func) = post_return {
                    ...
                    unsafe {
                        crate::Func::call_unchecked_raw(
                            &mut cx,
                            func.as_non_null(),
                            ptr::slice_from_raw_parts(&arg, 1).cast_mut(),
                        )?;
                    }
                }
